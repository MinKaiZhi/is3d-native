...
def load_train_config(path: Path) -> TrainConfig:
    ...
    return TrainConfig(
        ...
        checkpoint_every_steps=payload.get("checkpoint_every_steps", 0),
        checkpoint_keep_last=payload.get("checkpoint_keep_last", 0),
        ...
        val_shards=payload.get("val_shards"),
        val_key_include=payload.get("val_key_include", "__images__"),
        dataloader_workers=payload.get("dataloader_workers", 4),
        wds_shuffle_buffer=payload.get("wds_shuffle_buffer", 1024),
        val_wds_shuffle_buffer=payload.get("val_wds_shuffle_buffer", 0),
        eval_every_steps=payload.get("eval_every_steps", 0),
        val_batches=payload.get("val_batches", 20),
    )
